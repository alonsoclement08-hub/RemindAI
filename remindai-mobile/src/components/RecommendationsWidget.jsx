...
function RecoCard({ rec, onCreateReminder }) {
  ...
  return (
    <View style={styles.card}>
      ...
      <View style={styles.cardRight}>
        <Text style={[styles.confidence, { color: CONFIDENCE_COLOR(rec.confidence) }]}>
...
export default function RecommendationsWidget({ onReminderCreated }) {
  ...
  const handleCreateReminder = useCallback(async (rec) => {
    // Build a time for today at the suggested hour
    let scheduledAt = null;
    if (rec.suggestedTime) {
      const [h, m] = rec.suggestedTime.split(':').map(Number);
      const d = new Date();
      d.setHours(h, m, 0, 0);
      if (d < new Date()) d.setDate(d.getDate() + 1); // push to tomorrow if past
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        <View>
          ...
          <Text style={styles.subtitle}>Basé sur tes habitudes</Text>
        </View>
      ...
      {data.recommendations.map((rec, i) => (
        <RecoCard key={i} rec={rec} onCreateReminder={handleCreateReminder} />
      ))}
...
const styles = StyleSheet.create({
  ...
  createBtn: {
    backgroundColor: '#7F77DD', paddingHorizontal: 10, paddingVertical: 6,
    borderRadius: 8,
  },
